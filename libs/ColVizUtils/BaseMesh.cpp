...
namespace NSUtils
{
  ...
  unsigned int BaseMesh::calcNumTri ( )
  {
    ...
    {
      if ( mMesh->valence( cfi.handle( ) ) == 3 ) ++numTri;
    }
    mNumTris = numTri;
    return mNumTris;
  }
  ...
  unsigned int BaseMesh::calcNumQuads ( )
  {
    ...
    for ( MeshDef::ConstFaceIter cfi = mMesh->faces_sbegin( );
        cfi != mMesh->faces_end( ); ++cfi )
    {
      if ( mMesh->valence( cfi.handle( ) ) == 4 ) ++numQuads;
    }
    mNumQuads = numQuads;
    return mNumQuads;
  }
  ...
  void BaseMesh::calcTriIndices ( )
  {
    ...
    {
      if ( mMesh->valence( f.handle( ) ) != 3 ) // ignore non-triangles
      continue;
      for ( v = mMesh->cfv_iter( f.handle( ) ); v; ++v )
      {
        mTriIndices.push_back( v.handle( ).idx( ) );
      }
    }
  ...
  void BaseMesh::calcQuadIndices ( )
  {
    ...
    for ( f = mMesh->faces_begin( ); f != fEnd; ++f )
    {
      if ( mMesh->valence( f.handle( ) ) != 4 ) // ignore non-quads
      continue;
      for ( v = mMesh->cfv_iter( f.handle( ) ); v; ++v )
      {
        mQuadIndices.push_back( v.handle( ).idx( ) );
      }
    }
