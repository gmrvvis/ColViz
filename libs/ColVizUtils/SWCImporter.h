...
namespace NSUtils
{
  ...
  class SWCImporter
  {
      ...
      //Load the file
      void loadFile ( string fileName, bool pApplyStdDims,
      ...
      bool isIdInContainer ( SWCNode pId,
    ...
    public:
       ...
       * Default constructor
       */
      SWCImporter ( string fileName, bool pApplyStdDims = false,
          bool pApplyReplicantNodeTest = false );
      ...
      SWCNode getElementAt ( int i ) const;

      void modifElement ( unsigned int pId, SWCNode pNewValue );

      SWCConnection getConnectionAt ( int i );

      SWCBranch getBranchAt ( int i )
      {
        return mBranches[i];
      }
      ;
      ...
      //Get number of dendritic terminations
      const int getDendriticTermination ( int index ) const;
      ...
      const int getSomaNodesDiscarded ( ) const
      {
        return mSomaNodesDiscarded;
      }
  };
}
