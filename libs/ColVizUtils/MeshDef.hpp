...
namespace NSBaseMesh
{
  ...
  void assignDefaultColor ( MeshDef* mesh, const Iter& begin, const Iter& end,
      MeshDef::Color pColor )
  {
    for ( Iter i = begin; i != end; ++i )
    {
      mesh->set_color( i.handle( ), pColor );
    }
  }

  template<typename VertexHandle>
  void assignDefaultColor ( MeshDef* mesh, const VertexHandle & begin,
      const int end, MeshDef::Color pColor )
  {
    for ( int i = 0; i < end; ++i )
    {
