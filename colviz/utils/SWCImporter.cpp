...
namespace NSUtils
{
  ...
  void SWCImporter::SomaThreeCrazyNodesTest ( )
  {
            ...
            == preProcessNodeCollection[3].position[2] ) )
    {
      ...
      for ( unsigned int i = ( lNumEles + 1 ); i < lNumNodes; i++ )
      {
        ...
        if ( lNodeAux.parent <= int( lNumEles ))
        {
  ...
  void SWCImporter::SomaBeltTest ( )
  {
    ...
    if ( lNumEles > 1 )
    {
      ...
      for ( unsigned int i = 0; i < lNumEles; i++ )
      {
  ...
  int SWCImporter::getDendriticTermination ( unsigned int index ) const
  {
  ...
  void SWCImporter::exportToFile ( std::string pFile )
  {
    ...
    for ( size_t i = 2; i < nodeCollection.size( ); ++i )
    {
      ...
      float pDendriticModifier, float pApicalModifier, float pValBase )
  {
    ...
    for ( size_t i = 1; i < nodeCollection.size( ); ++i )
    {
      nodeAux = nodeCollection.at( i );
